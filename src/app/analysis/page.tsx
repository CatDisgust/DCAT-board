  ...
  return (
    <AppShell demo={demo}>
      ...
      <div className="analysis-grid">
        <article className="surface analysis-summary"><p className="card-kicker">体重方向</p><strong className="big">{trendText[analysis.weight.trend]}</strong><p>{analysis.weight.change === null ? "需要两个可比较窗口" : `${analysis.weight.change > 0 ? "+" : ""}${analysis.weight.change} kg · 7 日均值变化`}</p></article>
        <article className="surface analysis-summary"><p className="card-kicker">平均睡眠</p><strong className="big">{analysis.sleep.averageMinutes ? `${Math.floor(analysis.sleep.averageMinutes / 60)}h ${analysis.sleep.averageMinutes % 60}m` : "—"}</strong><p>n={analysis.sleep.sample} · 最近 7 天</p></article>
        <article className="surface analysis-summary"><p className="card-kicker">边界遵守率</p><strong className="big">{analysis.boundary.adherenceRate ?? "—"}%</strong><p>n={analysis.boundary.sample} · 最近 7 天</p></article>
      </div>
      <article className="surface chart-card"><h2>体重趋势</h2><p className="page-description">移动平均用于抵消单日水分、钠摄入和测量时点造成的噪声。</p><WeightChart records={records.slice(-14)} /></article>
